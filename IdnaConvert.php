 ...
 * @version 2008
 *
 * @contributor Foma Tuturov (update to PHP8+ )
 */
class IdnaConvert
{
    // NP See below
    ...
    protected string $punycodePrefix = 'xn--';
    protected int $invalidUcs = 0x80000000;
    protected int $maxUcs = 0x10FFFF;
    protected int $base = 36;
    protected int $tMin = 1;
    protected int $tMax = 26;
    protected int $skew = 38;
    protected int $damp = 700;
    protected int $initialBias = 72;
    protected int $initialN = 0x80;
    protected int $sBase = 0xAC00;
    protected int $lBase = 0x1100;
    protected int $vBase = 0x1161;
    protected int $tBase = 0x11A7;
    protected int $lCount = 19;
    protected int $vCount = 21;
    protected int $tCount = 28;
    protected int $nCount = 588;   // vCount * tCount
    protected int $sCount = 11172; // lCount * tCount * vCount
    protected bool|string $error = false;

    protected static ?bool $mbStringOverload = null;

    // See {@link setParameter()} for details of how to change the following
    // settings from within your script / application
    protected string $apiEncoding = 'utf8';   // Default input charset is UTF-8
    protected bool $allowOverlong = false;  // Overlong UTF-8 encodings are forbidden
    protected bool $strictMode = false;     // Behave strict or not
    protected int $idnVersion = 2003;      // Can be either 2003 (old, default) or 2008

    protected int|float $sLast;
    protected static $safeChar;
    protected static $safeMode;
     ...
     */
    public function setParameter(mixed $option, bool|string $value = false): bool
    {
        if (!is_array($option)) {
            $option = [$option => $value];
        }
        foreach ($option as $k => $v) {
            switch ($k) {
                case 'encoding':
                    ...
                    break;
                case 'overlong':
                    $this->allowOverlong = (bool)$v;
                    break;
                case 'strict':
                    $this->strictMode = (bool)$v;
                    break;
                case 'idn_version':
     ...
     */
    public function decode(string $input, $oneTimeEncoding = false): bool|array|string
    {
        // Optionally set
        if ($oneTimeEncoding) {
        ...
        if (strpos($input, '@')) { // Maybe it is an email address
            ...
            [$emailPref, $input] = explode('@', $input, 2);
            $arr = explode('.', $input);
            foreach ($arr as $k => $v) {
            ...
            if (isset($parsed['host'])) {
                ...
                $return = $this->getParsedData($arr, $parsed);
     ...
     */
    public function encode(string $decoded, string $oneTimeEncoding = ''): bool|string
    {
        ...
        switch (!empty($oneTimeEncoding) ? $oneTimeEncoding : $this->apiEncoding) {
            ...
            case 'ucs4_array':
                break;
            default:
                $this->useError('Unsupported input format: ' . ($oneTimeEncoding ?: $this->apiEncoding));
                return false;
        }
        ...
        if ($lastBegin) {
            ...
            return $output;
        } else if ($output = $this->useEncode($decoded)) {
            return $output;
        } else {
     ...
     * @since 0.6.4
     */
    public function encodeUri(string $uri): bool|string
    {
     ...
     * @return bool|string
     */
    protected function useDecode(string $encoded): bool|string
    {
        $decoded = [];
        ...
        for ($encoIdx = ($delimPos) ? ($delimPos + 1) : 0; $encoIdx < $encoLen; ++$decoLen) {
            ...
            $bias = $this->useAdapt($idx - $old_idx, $decoLen + 1, (int)$isFirst);
            $isFirst = false;
            $char += (int)($idx / ($decoLen + 1));
            $idx %= ($decoLen + 1);
            if ($decoLen > 0) {
                // Make room for the decoded char
                for ($i = $decoLen; $i > $idx; $i--) {
     ...
     * @param array|string $decoded
     * @return false|string
     */
    protected function useEncode(array|string $decoded): false|string
    {
        ...
        // Do NAMEPREP
        $decoded = $this->useNamePrepare($decoded);
            ...
            for ($i = 0; $i < $decoLen; $i++) {
                if ($decoded[$i] < $curCode) {
                    ...
                    for ($q = $delta, $k = $this->base; 1; $k += $this->base) {
                        ...
                        $encoded .= $this->useEncodeDigit((int)($t + (($q - $t) % ($this->base - $t)))); //v0.4.5 Changed from ceil() to intval()
                        $q = (int)(($q - $t) / ($this->base - $t));
                    }
                    $encoded .= $this->useEncodeDigit($q);
                    $bias = $this->useAdapt($delta, $codecount + 1, (int)$isFirst);
                    $codecount++;
                    $delta = 0;
                    $isFirst = false;
                }
     ...
     * @return int
     */
    protected function useAdapt(int $delta, int $npoints, int $isFirst): int
    {
        $delta = (int)($isFirst ? ($delta / $this->damp) : ($delta / 2));
        $delta += (int)($delta / $npoints);
        for ($k = 0; $delta > (($this->base - $this->tMin) * $this->tMax) / 2; $k += $this->base) {
            $delta = (int)($delta / ($this->base - $this->tMin));
        }
     ...
     * @return string
     */
    protected function useEncodeDigit(int $d): string
    {
     ...
     * @return int
     */
    protected function useDecodeDigit(int $cp): int
    {
        $cp = ord($cp);
     ...
     * @param string $error
     */
    protected function useError(string $error = ''): void
    {
        $this->error = $error;
    }
     ...
     * @return   array|bool|integer[]   Unicode Characters, Nameprep'd
     */
    protected function useNamePrepare(array $input): bool|array
    {
        $output = [];
        //
        // Mapping
     ...
     */
    protected function useHangulDecompose(int $char): array
    {
        ...
        $T = (int)($this->tBase + $sindex % $this->tCount);
        if ($T !== $this->tBase) {
            $result[] = $T;
        }
        return $result;
     ...
     */
    protected function useHangulCompose(array $input): array
    {
        $inp_len = count($input);
        if (!$inp_len) {
            return [];
     ...
     */
    protected function getCombiningClass(int $char): int
    {
     ...
     */
    protected function applyCanonicalOrdering(array $input): array
    {
        $swap = true;
        $size = count($input);
        while ($swap) {
     ...
     * @return   bool|int|string      Ordered USC4 sequence
     */
    protected function combine(array $input): bool|int|string
    {
        $inpLen = count($input);
        foreach (self::$NP['replacemaps'] as $npSrc => $npTarget) {
            if ($npTarget[0] !== $input[0]) {
                continue;
            }
            if (count($npTarget) !== $inpLen) {
                continue;
            }
            $hit = false;
     ...
     * @return array|bool
     */
    protected function utf8ToUcs4(string $input): array|bool
    {
        $output = [];
        $outLen = 0;
     ...
     * @return string|bool
     */
    protected function ucs4ToUtf8(array $input): bool|string
    {
        $output = '';
     ...
     * @return string
     */
    protected function ucs4ToUcs4String(array $input): string
    {
        $output = '';
     ...
     * @return array|false
     */
    protected function ucs4StringToUcs4(string $input): false|array
    {
        $output = [];
        $inpLen = self::byteLength($input);
     ...
     */
    protected static function byteLength(string $string): int
    {
    ...
    /**
     * Attempts to return a concrete IDNA logger.
     *
     * @param array $params Set of parameters
     * @return IdnaConvert
     * @access public
     */
    public function getInstance(array $params = []): IdnaConvert
    {
        return new IdnaConvert($params);
    }

    /**
     * Attempts to return a concrete IDNA logger for either php4,php5 оr php7,
     * only creating a new logger if no IDNA logger with the same
    ...
    /**
     * @param array $arr
     * @param int|array|string $parsed
     * @return string
     */
    private function getParsedData(array $arr, int|array|string $parsed): string
    {
